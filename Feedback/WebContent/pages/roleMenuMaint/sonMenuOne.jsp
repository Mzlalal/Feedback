...
<script type="text/javascript">
	window.onload=function(){
  		var menu = document.getElementsByNames("roleMenu.vf");
  		alert();
}
</script>
</head>
					...
					<div id="example_wrapper" class="dataTables_wrapper form-inline"
						...
						<table cellpadding="0" cellspacing="0" border="0"
							...
							<tbody role="alert" aria-live="polite" aria-relevant="all" id="baseTab">
								<c:forEach	var="sonMenuOne" items="${sonMenuOne}" varStatus="status">
									<tr class="gradeA odd">
										  <td class="sorting"><c:out value="${status.index+1}"/></td>
										  <td class="sorting"><c:out value="${sonMenuOne.menuId}" /></td>
										  <td class="sorting"><c:out value="${sonMenuOne.menuName}" /></td>
										  <td class="sorting">
										  <c:if test="${sonMenuOne.parentMenuId==1}">
										   		系统信息维护
										  </c:if></td>
										  <td class="sorting">		
	           								  <input type="checkbox"  name="menu"
	           								   <c:if test="${sonMenuOne.validFlag==1}"> checked="checked" value="1"</c:if>/>
										  </td>
										  <td class="sorting"><a class="btn btn-primary btn-xs" href="roleMenuAction_save?menuId=${sonMenuOne.menuId}&bool=${sonMenuOne.validFlag==1}" >保存 </a> </td> 
									</tr>
								</c:forEach>
