					...
					<div id="example_wrapper" class="dataTables_wrapper form-inline"
						...
						<table cellpadding="0" cellspacing="0" border="0"
							...
							<tbody role="alert" aria-live="polite" aria-relevant="all" id="baseTab">
								<c:forEach	var="sonMenuTwo" items="${sonMenuTwo}" varStatus="status">
									<tr class="gradeA odd">
										  <td class="sorting"><c:out value="${status.index+1}"/></td>
										  <td class="sorting"><c:out value="${sonMenuTwo.menuId}" /></td>
										  <td class="sorting"><c:out value="${sonMenuTwo.menuName}" /></td>
										  <td class="sorting">
										  <c:if test="${sonMenuTwo.parentMenuId==2}">
										   		基础信息维护
										  </c:if></td>
										  <td class="sorting">		
	           								  <input type="checkbox"  name="menu"
	           								   <c:if test="${sonMenuTwo.validFlag==1}"> checked="checked" value="1"</c:if>/>
										  </td>
										  <td class="sorting"><a class="btn btn-primary btn-xs" href="roleMenuAction_save?menuId=${sonMenuTwo.menuId}&bool=${sonMenuTwo.validFlag==1}" >保存 </a> </td> 
									</tr>
								</c:forEach>
